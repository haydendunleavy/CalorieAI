...
// Blur fallback: Expo Go on emulator = no blur
const blurSupported =
  Platform.OS !== 'web' &&
...
export default function HomeScreen({ navigation, theme }) {
  ...
  // Animate card → center → show edit/delete
  const handleCardPress = (meal) => {
    ...
    const absoluteY = layout.y - scrollY;
    ...
    <>
      <View style={styles.mealHeader}>
        ...
        <View style={[styles.calorieBadge, { backgroundColor: theme.accentLight }]}>
          <Text style={[styles.calorieBadgeText, { color: theme.accent }]}>
            {meal.calories} cal
          </Text>
        </View>
      </View>
      ...
      <View style={styles.mealBars}>
        <View style={styles.mealBarRow}>
          ...
          <Text style={[styles.mealBarLabel, { color: theme.textSecondary }]}>
            {meal.protein}g
          </Text>
        </View>
        <View style={[styles.barBg, { backgroundColor: theme.ringBg }]}>
          <View
            style={[
              styles.barFill,
              {
                width: `${Math.min((meal.protein / proteinGoal) * 100, 100)}%`,
        ...
        <View style={[styles.barBg, { backgroundColor: theme.ringBg }]}>
          <View
            style={[
              styles.barFill,
              {
                width: `${Math.min((meal.carbs / carbsGoal) * 100, 100)}%`,
        ...
        <View style={[styles.barBg, { backgroundColor: theme.ringBg }]}>
          <View
            style={[
              styles.barFill,
              {
                width: `${Math.min((meal.fat / fatGoal) * 100, 100)}%`,
  ...
  return (
    <SafeAreaView style={[styles.screen, { backgroundColor: theme.background }]}>
      <ScrollView
        ref={scrollRef}
        onScroll={(e) => {
          scrollRef.current._scrollPos = e.nativeEvent.contentOffset.y;
        }}
      ...
      >
        ...
        <Progress.Circle
          ...
          textStyle={{
            color: isOverGoal ? '#ef4444' : theme.text,
            fontWeight: '700',
          }}
        />
        ...
        <View style={styles.calorieSummary}>
          <View style={styles.calorieStat}>
            <Text style={[styles.calorieStatNum, { color: theme.text }]}>
              {calorieGoal}
            </Text>
            <Text
              style={[styles.calorieStatLabel, { color: theme.textSecondary }]}
            >
              Goal
            </Text>
          </View>
          <View style={styles.calorieStat}>
            <Text style={[styles.calorieStatNum, { color: theme.accent }]}>
              {caloriesEaten}
            </Text>
            <Text
              style={[styles.calorieStatLabel, { color: theme.textSecondary }]}
        ...
        >
          <View style={styles.macroRow}>
            ...
            <Text
              style={[styles.macroValue, { color: theme.textSecondary }]}
            >{`${proteinEaten}g / ${proteinGoal}g`}</Text>
          </View>
          <Progress.Bar
        ...
        <View style={styles.mealsContainer}>
          <View style={styles.mealsTitleRow}>
            ...
            {meals.length > 0 && (
              <Text
                style={[styles.mealsHint, { color: theme.textTertiary }]}
              >
                Tap a meal to edit
              </Text>
            )}
          ...
          {meals.length === 0 ? (
            ...
            meals.map((meal) => (
              <View
                key={meal._id}
                onLayout={(e) =>
                  handleCardLayout(meal._id, e.nativeEvent.layout)
                }
              >
                <TouchableOpacity
                  activeOpacity={0.7}
            ...
            >
              ...
              <View style={styles.cloneActions}>
                <TouchableOpacity
                  onPress={() => {
                    setAnimMeal(null);
                    navigation.navigate('EditMeal', { meal: animMeal.meal });
                  }}
                >
                ...
                <TouchableOpacity
                  onPress={async () => {
                    await api.deleteMeal(animMeal.meal._id);
                    setAnimMeal(null);
                    loadMeals();
                  }}
                >
